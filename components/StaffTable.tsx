  ...
  return (
    <div className="space-y-4">
      ...
      <div className="md:hidden space-y-2">
        {pagedStaff.map(s => (
          <div key={s.id} className={`bg-white rounded-2xl border border-spring-pink-border p-4 shadow-sm ${!s.is_active ? 'opacity-50' : ''}`}>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <p className="text-sm font-bold text-spring-text">{s.name}</p>
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    ...
                    {s.specialties.length > 3 && <span className="text-[10px] text-spring-text-light">+{s.specialties.length - 3}</span>}
                  </div>
                  <p className="text-xs text-spring-text-light mt-1 truncate">{s.email}</p>
                  {s.phone && <p className="text-xs text-spring-text-light">{s.phone}</p>}
                </div>
              </div>
              <button
                onClick={e => { e.stopPropagation(); toggleActive(s.id, !s.is_active) }}
                className={`w-10 h-5 rounded-full transition-colors shrink-0 mt-1 ${s.is_active ? 'bg-spring-emerald' : 'bg-gray-200'}`}
              >
                <span className={`block w-4 h-4 bg-white rounded-full mx-0.5 transition-transform shadow-sm ${s.is_active ? 'translate-x-5' : 'translate-x-0'}`} />
              </button>
            </div>
          </div>
        ))}
        {filteredStaff.length === 0 && (
          <div className="py-10 text-center text-spring-text-light text-sm">
